...
class Mdl_Quote_Item_Amounts extends CI_Model
{
    ...
    public function calculate($item_id)
    {
        ...
        $item_discount_amount = (empty($item->item_discount_amount) ? 1 : ($item->item_discount_amount / 100));
        $tax_rate_percent     = $item->item_tax_rate_percent / 100;

        // pZ:
        $item_subtotal       = $item->item_price * $item->item_quantity;
        $item_discount_total = $item_subtotal * $item_discount_amount;

        // pZ: calculate tax after apply discount
        $subtotal       = $item_subtotal - $item_discount_total;
        $item_tax_total = $subtotal * $tax_rate_percent;
        $item_total     = $subtotal + $item_tax_total;
