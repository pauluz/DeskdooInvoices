...
class Settings extends Admin_Controller
{
    ...
    public function index()
    {
        if ($this->input->post('settings')) {
            ...
            // pZ: validate BCC email to Admin
//            if ($this->mdl_settings->run_validation('validation_bcc_email')) {
//                echo '<pre>'; var_dump($settings); echo '</pre>'; die(' -pauluZ-35');
//            }
            ...
            foreach ($settings as $key => $value) {
                ...
                // pZ: na razie wyłączam bo potrzebna jest spacja w rozdzielaniu tysięcy
//                $value = trim($value);
        ...
        // pZ: set organization_name if not exists yet
        $organization_name = $this->mdl_settings->setting('organization_name');
        if (empty($organization_name)) {
            $organization_name = $this->session->userdata('company_name');
            $this->mdl_settings->set_setting('organization_name', $organization_name);
        }
