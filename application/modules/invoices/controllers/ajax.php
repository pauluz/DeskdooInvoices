...
class Ajax extends Admin_Controller
{
    ...
    public function save()
    {
        ...
        if ($this->mdl_invoices->run_validation('validation_rules_save_invoice')) {
            ...
            if ($this->config->item('disable_read_only') == FALSE) {
                ...
                foreach($invoice_statuses as $key => $status) { // draft(not needed), sent, viewed, paid
                    if (($invoice_status == $key) && ($read_only_toggle == $status['class'])) {
    ...
    public function change_client()
    {
        ...
        // Get the client ID
        $client_name = $this->input->post('client_name');
        $client = $this->mdl_clients->where('client_name', $this->db->escape_str($client_name))->get()->row();
