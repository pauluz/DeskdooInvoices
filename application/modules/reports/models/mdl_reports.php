...
class Mdl_Reports extends CI_Model
{
    ...
    {
        // pZ:
        $company_id = 'ip_invoices.company_id = ' . $this->session->userdata('deskdoo_company_id') . ' AND';
        ...
        if ($from_date and $to_date) {
            ...
            $this->db->select("(SELECT COUNT(*) FROM ip_invoices WHERE ${company_id} ip_invoices.client_id = ip_clients.client_id and invoice_date_created >= " . $this->db->escape($from_date) . " and invoice_date_created <= " . $this->db->escape($to_date) . ") AS invoice_count");
            $this->db->select("(SELECT SUM(invoice_item_subtotal) FROM ip_invoice_amounts WHERE ip_invoice_amounts.invoice_id IN (SELECT invoice_id FROM ip_invoices WHERE ${company_id} ip_invoices.client_id = ip_clients.client_id and invoice_date_created >= " . $this->db->escape($from_date) . " and invoice_date_created <= " . $this->db->escape($to_date) . ")) AS sales");
            ...
            $this->db->where('client_id IN (SELECT client_id FROM ip_invoices WHERE ' . $company_id . ' invoice_date_created >=' . $this->db->escape($from_date) . ' and invoice_date_created <= ' . $this->db->escape($to_date) . ')');
        } else {
            $this->db->select('(SELECT COUNT(*) FROM ip_invoices WHERE ' . $company_id . ' ip_invoices.client_id = ip_clients.client_id) AS invoice_count');
            $this->db->select('(SELECT SUM(invoice_item_subtotal) FROM ip_invoice_amounts WHERE ip_invoice_amounts.invoice_id IN (SELECT invoice_id FROM ip_invoices WHERE ' . $company_id . ' ip_invoices.client_id = ip_clients.client_id)) AS sales');
            $this->db->select('(SELECT SUM(invoice_total) FROM ip_invoice_amounts WHERE ip_invoice_amounts.invoice_id IN (SELECT invoice_id FROM ip_invoices WHERE ' . $company_id . ' ip_invoices.client_id = ip_clients.client_id)) AS sales_with_tax');
            $this->db->where('client_id IN (SELECT client_id FROM ip_invoices WHERE ' . $company_id . ' 1)');
        }
    ...
    public function invoice_aging()
    {
        ...
        $this->db->select('client_name');
        $this->db->select('(SELECT SUM(invoice_balance) FROM ip_invoice_amounts WHERE invoice_id IN (SELECT invoice_id FROM ip_invoices WHERE ' . $company_id . ' ip_invoices.client_id = ip_clients.client_id AND invoice_date_due <= DATE_SUB(NOW(),INTERVAL 1 DAY) AND invoice_date_due >= DATE_SUB(NOW(), INTERVAL 15 DAY))) AS range_1', FALSE);
        $this->db->select('(SELECT SUM(invoice_balance) FROM ip_invoice_amounts WHERE invoice_id IN (SELECT invoice_id FROM ip_invoices WHERE ' . $company_id . ' ip_invoices.client_id = ip_clients.client_id AND invoice_date_due <= DATE_SUB(NOW(),INTERVAL 16 DAY) AND invoice_date_due >= DATE_SUB(NOW(), INTERVAL 30 DAY))) AS range_2', FALSE);
        $this->db->select('(SELECT SUM(invoice_balance) FROM ip_invoice_amounts WHERE invoice_id IN (SELECT invoice_id FROM ip_invoices WHERE ' . $company_id . ' ip_invoices.client_id = ip_clients.client_id AND invoice_date_due <= DATE_SUB(NOW(),INTERVAL 31 DAY))) AS range_3', FALSE);
        $this->db->select('(SELECT SUM(invoice_balance) FROM ip_invoice_amounts WHERE invoice_id IN (SELECT invoice_id FROM ip_invoices WHERE ' . $company_id . ' ip_invoices.client_id = ip_clients.client_id AND invoice_date_due <= DATE_SUB(NOW(), INTERVAL 1 DAY))) AS total_balance', FALSE);
    ...
    {
        // pZ:
        $company_id = 'inv.company_id = ' . $this->session->userdata('deskdoo_company_id') . ' AND';
